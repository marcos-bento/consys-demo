...
export default function Comercial() {
  ...
  const filteredPropostas = useMemo(() => {
    return propostas.filter(proposta => {
      const matchesSearch = proposta.cliente.toLowerCase().includes(search.toLowerCase()) ||
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold">Comercial / Propostas</h1>
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
        ...
        <Button onClick={() => router.push('/comercial/nova')}>
          Nova Proposta
        </Button>
      </div>
          ...
          <Table>
            ...
            <TableBody>
              {filteredPropostas.map((proposta) => {
                const total = proposta.itens.reduce((sum, item) => sum + item.qtd * item.valorUnit, 0) - proposta.desconto;
                return (
                    ...
                    <TableCell>
                      <div className="flex space-x-2">
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => router.push(`/comercial/${proposta.id}`)}
                        >
